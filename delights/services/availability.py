...
def update_dishes_for_ingredient(ingredient: Ingredient) -> None:
    ...
    dishes = (
        Dish.objects.filter(recipe_requirements__ingredient=ingredient)
        .distinct()
...
def update_menus_for_dish(dish: Dish) -> None:
    ...
    menus = Menu.objects.filter(dishes=dish).prefetch_related("dishes")
    ...
    if menus_to_update:
        Menu.objects.bulk_update(menus_to_update, ["cost", "is_available"])
