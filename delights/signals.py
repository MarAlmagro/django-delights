...
def ingredient_saved(sender, instance, **kwargs):
    ...
    if (
        ...
        or "quantity_available" in update_fields
        or "price_per_unit" in update_fields
    ):
...
@receiver([post_save, post_delete], sender="delights.Purchase")
def invalidate_dashboard_cache_on_purchase(sender, **kwargs):
    """Invalidate dashboard cache when purchases change."""
    cache.delete("dashboard_metrics")


@receiver([post_save, post_delete], sender="delights.PurchaseItem")
def invalidate_dashboard_cache_on_purchase_item(sender, **kwargs):
    """Invalidate dashboard cache when purchase items change."""
    cache.delete("dashboard_metrics")
