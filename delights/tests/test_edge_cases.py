...
class TestEdgeCases:
    ...
    def test_ingredient_with_no_dishes(self, db):
        ...
        ingredient = IngredientFactory()
        assert ingredient.recipe_requirements.count() == 0
    ...
    def test_recipe_requirement_zero_quantity(self, db):
        ...
        recipe = RecipeRequirementFactory(
            dish=dish, ingredient=ingredient, quantity_required=Decimal("0")
        )
    ...
    def test_precision_in_decimal_calculations(self, db):
        ...
        ingredient = IngredientFactory(
            quantity_available=Decimal("10.123456"), price_per_unit=Decimal("5.987654")
        )
    ...
    def test_ingredient_unit_variations(self, db):
        ...
        from delights.models import Unit

        units = ["kg", "g", "lb", "oz", "L", "mL", "cup", "tbsp", "tsp", "piece"]

        for unit_name in units:
            unit = Unit.objects.create(
                name=unit_name, description=unit_name, is_active=True
            )
            ingredient = IngredientFactory(unit=unit)
            assert ingredient.unit.name == unit_name
    ...
    def test_same_ingredient_multiple_times_in_dish(self, db):
        ...
        RecipeRequirementFactory(
            dish=dish, ingredient=ingredient, quantity_required=Decimal("5")
        )
        ...
        try:
            with transaction.atomic():
                RecipeRequirementFactory(
                    dish=dish, ingredient=ingredient, quantity_required=Decimal("3")
                )
...
class TestBoundaryConditions:
    ...
    def test_minimum_decimal_values(self, db):
        ...
        ingredient = IngredientFactory(
            quantity_available=Decimal("0.01"), price_per_unit=Decimal("0.01")
        )
    ...
    def test_fractional_quantities(self, db):
        ...
        recipe = RecipeRequirementFactory(
            ingredient=ingredient, quantity_required=Decimal("0.111")
        )
