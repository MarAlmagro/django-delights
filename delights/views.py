...
def purchase_finalize(request):
    ...
    try:
        ...
        for item in purchase_data:
            ...
            for (
                requirement
            ) in dish.recipe_requirements.select_for_update().select_related(
                "ingredient"
            ):
        ...
        for ing_id, ing_data in ingredients_needed.items():
            ...
            if locked_ingredient.quantity_available < total_needed:
                messages.error(
                    request,
                    f"Insufficient inventory: {locked_ingredient.name} (need {total_needed}, have {locked_ingredient.quantity_available}).",
                )
                return redirect("purchases:confirm")
